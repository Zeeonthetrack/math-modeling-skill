...
【移植说明】本脚本移植自网络公开 skill "cumcm-aigc-reduce-skill"（scripts/aigc_scan.py），
逻辑保持不变，仅将报告中的指引路径改指本 skill 的 写作规范.md。
定位：论文手交付前的启发式辅助扫描——只提示需要人工判定的位置，
不作为否决依据；词表为"常见 AI 高频特征示例"，非逐词禁令。
最终判定标准见 references/roles/论文手/references/写作规范.md 第一节第 6 条。
...
def analyze_dash_density(paragraphs: list[str]) -> dict:
    ...
    return {
        ...
        'risk': f'⚠️ {over_limit_paras} 段存在破折号' if over_limit_paras else '正常'
    }
...
def print_report(result: dict):
    ...
    if co['count'] > 0 or dd['over_limit_paras'] > 0:
        ...
        print("     请对照 写作规范.md 第一节第 6 条改回书面学术表达。")
