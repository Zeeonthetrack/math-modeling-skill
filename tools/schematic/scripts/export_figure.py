...
def main():
    ...
    if not cli:
        # Windows 用户级安装默认不进 PATH，探测常见安装路径
        candidates = [
            pathlib.Path.home() / 'AppData' / 'Local' / 'Programs' / 'draw.io' / 'draw.io.exe',
            pathlib.Path('C:/Program Files/draw.io/draw.io.exe'),
        ]
        for c in candidates:
            if c.exists():
                cli = str(c)
            ...
            sys.exit('未找到 drawio 命令行。\n'
                     ...
                     '  Windows: 安装 draw.io 桌面版后确保在 PATH 中，或装到上述标准路径\n'
