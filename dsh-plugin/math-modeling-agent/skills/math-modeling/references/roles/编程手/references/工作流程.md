...
6. 论文所需示意图（技术路线图、思路/算法流程图、模型机制示意）与数据图分开产出：按 `tools/schematic/SKILL.md` 用该工具绘制可编辑 `.drawio`，经其 `check_layout.py` 体检并人工核对渲染图后导出 PNG（建议 `-s 3` 及以上倍率）+ 矢量 PDF/SVG。示意图命名 `diagram_qN_*`（全文级 `diagram_all_*`），内容只能取自题目分析报告、模型说明和真实结果，禁止编造；计入论文正式图但只补充不替代，不占用三类图配额，`figure_audit.py` 按 diagram 类别校验配对。
...
└── figures/
    ...
    ├── diagram_q1_* / diagram_all_*        # 示意图（可选）：.drawio 源 + .png + 矢量 .pdf/.svg
    └── 图表面板.html
```
