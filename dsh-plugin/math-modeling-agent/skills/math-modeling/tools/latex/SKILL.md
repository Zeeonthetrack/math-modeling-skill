...
CUMCM 的约 9000 字词单位（典型 9,000~15,000）、约 20 页、5 个公式和 3 个表只是可覆盖的完整度质量目标（字数区间取自近年国一论文实测，禁止凑字数填充）。CUMCM 与 MCM/ICM 均默认至少 8 幅图；页数上限等官方硬约束必须从目标届次规则读取后通过参数传入。MCM/ICM 不内置永久页数阈值。
