...
同时记录两类篇幅约束：一类是当届官方页数、摘要和文件限制，必须满足；另一类是论文完整度质量目标，可以由用户覆盖。CUMCM 未给出更具体要求时，可按典型 9,000~15,000 字词单位、约 20 页规划（近年国一论文实测区间，禁止凑字数填充），但不得称为官方最低要求。以 2026 年官方规范为例，正文不超过 30 页。
